...
export async function getServerSideProps(context) {
  ...
  for (let item of products) {
    // console.log('ok11', hoods);
    if (item.title in hoods) {
      // Ensure 'color' is an array before using .push()
      if (Array.isArray(hoods[item.title].color) && item.availableQty > 0) {
      ...
      if (Array.isArray(hoods[item.title].size) && item.availableQty > 0) {
        if (!hoods[item.title].size.includes(item.size)) {
          hoods[item.title].size.push(item.size);
        }
      }
    } else {
      // Initialize hood entry
