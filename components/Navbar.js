...
const Navbar = ({ user, usertoken, cart, addToCart, removeFromCart, deleteCartItem, subTotal, logout }) => {
  const ref = useRef();
    ...
    <>
      ...
      <div className={`flex flex-col  items-center justify-start md:flex-row shadow-xl my-1 sticky top-0 bg-white ${!sidebar && 'overflow-hidden'}`}>
        ...
        <div className="cart flex absolute top-3 right-0 mx-8 text-2xl text-pink-500">
          ...
          {!usertoken.value && <Link href={'/login'} >
          ...
          <span className='flex  cursor-pointer  mx-2 text-2xl text-pink-500 z-30' onMouseOver={() => { setDropdown(true) }} >
            {usertoken.value && <MdAccountCircle className="mx-2 hover:text-pink-800" />}
          </span>
        ...
        <div ref={ref} className={`h-screen sidebar absolute overflow-y-scroll p-3 h- top-10 ${sidebar ? 'right-72 md:right-96' : '-right-96'} w-2/3 md:w-3/12 bg-pink-100 transition-all translate-x-full`}>
